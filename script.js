...
function setStale(value) {
  ...
  // "Bagud" er ubrugeligt uden et hvornår. Med klokkeslættet kan man selv
  // vurdere, om tallene er et kvarter eller fem timer gamle.
  const since = $("[data-stale-since]");
  if (since)
    since.textContent =
      value && state.updatedAt
        ? `Seneste kendte status er fra kl. ${formatClock(new Date(state.updatedAt))}.`
        : "";
...
function initSubmissionForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const media = form.elements.media_link;
    if (media?.value && !/^https?:\/\//i.test(media.value.trim())) {
      return fail(media, "Linket skal begynde med https://");
    }
    ...
    try {
      ...
      if (status) {
        status.textContent = saved
          ? "Vi kunne ikke få fat i serveren. Dit bidrag er gemt som kladde på denne enhed — tryk “Send igen” under kladderne, når du vil prøve igen."
...
function renderDrafts() {
  ...
  drafts.forEach((draft, index) => {
    ...
    // En kladde uden gensendelse er bare noget, brugeren har mistet. Derfor
    // ligger selve indsendelsen her, ikke kun i formularen.
    const retry = document.createElement("button");
    retry.type = "button";
    retry.className = "plain-button";
    retry.textContent = "Send igen";
    retry.setAttribute(
      "aria-label",
      `Send kladden “${title.textContent}” igen`,
    );
    retry.addEventListener("click", async () => {
      retry.disabled = true;
      retry.textContent = "Sender…";
      const payload = new FormData();
      for (const [key, value] of Object.entries(draft))
        if (key !== "created_at") payload.append(key, value);
      try {
        const response = await fetch(CONFIG.submissionEndpoint, {
          method: "POST",
          body: payload,
        ...
        setText("[data-drafts-status]", "Kladden er sendt afsted. Tak.");
        renderDrafts();
      } catch {
        ...
        setText(
          "[data-drafts-status]",
          "Serveren svarer stadig ikke. Kladden bliver liggende.",
        );
      }
    });
    ...
    item.append(title, meta, retry, remove);
    list.append(item);
  });
}
...
function initToggles() {
  ...
  bind("#spoiler-toggle", "spoiler-free", "rdr2thon-spoilerfree", () => {
    refreshSpoilers();
    renderJournal();
    // Livekortet i features.js kan ikke nøjes med at maskere: det skal tegne
    // helt om, fordi kommende ankre slet ikke må stå på kortet.
