...
/* DATA må ikke ligge i ROOT i produktion. Alt i ROOT bliver udgivet af en
   statisk host, og mappen indeholder adgangskodehash, sessionshemmelighed,
   IP-adresser fra indsendelser og stemmer. Sæt DATA_DIR til et sted uden for
   webroden — fx DATA_DIR=/var/lib/rdr2thon. Standarden ./data findes kun,
   fordi den er nem at komme i gang med lokalt. */
const DATA = path.resolve(process.env.DATA_DIR || path.join(ROOT, "data"));
const PORT = Number(process.env.PORT || 3000);
if (DATA.startsWith(ROOT) && !process.env.DATA_DIR)
  console.warn(
    "\n  ADVARSEL: data/ ligger i webroden. Udgiver du mappen som statisk\n" +
      "  site, offentliggør du adgangskodehash, sessionshemmelighed og\n" +
      "  indsendelser. Sæt DATA_DIR før produktion.\n",
  );
...
async function readJSON(file, fallback) {
  ...
  try {
    ...
    // Filen findes ikke endnu. Det er den normale førstegangstilstand.
    return fallback;
  }
  try {
    return JSON.parse(raw);
  } catch (error) {
    // Ødelagt JSON må ALDRIG bare blive til en tom normaltilstand: så ligner
    // datatab en ny installation, og næste skrivning overskriver resterne.
    // Vi lægger filen til side, så den kan reddes i hånden.
    const rescue = `${file}.ødelagt-${Date.now()}`;
    try {
      await fsp.rename(file, rescue);
    } catch {
      /* kan vi ikke engang omdøbe, er der intet at redde */
    }
    console.error(
      `\n  FEJL: ${path.basename(file)} kunne ikke læses (${error.message}).\n` +
        `  Filen er lagt til side som ${path.basename(rescue)}.\n` +
        `  Tjek den, før du fortsætter — der kan være gået data tabt.\n`,
    );
    return fallback;
  }
}
...
/* Én kø pr. fil. To samtidige stemmer læste før begge skrev, og den sidste
   overskrev den første — stemmer forsvandt. Tempfilnavnet indeholder også et
   tilfældigt led, fordi PID er det samme for alle samtidige requests i samme
   proces, så de ellers skrev i den samme tempfil. */
...
function writeJSON(file, value) {
  ...
  const next = previous.then(
    ...
    async () => {
      // Fejlede den forrige skrivning, må denne stadig få lov at prøve.
      const tmp = `${file}.${process.pid}.${crypto.randomBytes(4).toString("hex")}.tmp`;
      await fsp.writeFile(tmp, JSON.stringify(value, null, 2), "utf8");
      await fsp.rename(tmp, file);
    },
...
/** Læs, ændr, skriv — uden at nogen kan nå at komme imellem. Alle
 *  opdateringer af den samme fil stilles i den samme kø. */
function updateJSON(file, fallback, change) {
  const run = async () => {
...
/* ------------------------------------------------------------- afstemninger
   ...
   Stemmesedler gemmes som { ballots: { afstemning: { vælger: svar } } } og
   tælles op ved læsning. Gemmer man kun tal, kan man ikke se forskel på "en
   ny stemme" og "en der stemmer om", og så kan tællingen manipuleres.
   ========================================================================= */

/** Lovlige svar for en afstemning lige nu — udelukkende ud fra state. */
function legalOptions(state, pollId) {
  const labels = (list) =>
    (Array.isArray(list) ? list : [])
      .map((o) => (typeof o === "string" ? o : o && o.label))
      .filter((s) => typeof s === "string" && s.length > 0);

  if (pollId === "horse-name")
    return state && state.horse && state.horse.votingOpen
      ? labels(state.horse.options)
      : [];

  const prediction = ((state && state.predictions) || []).find(
    (p) => p && String(p.id) === pollId,
  );
  if (prediction && !prediction.closed && !prediction.resolved)
    return labels(prediction.options);

  const poll = state && state.poll;
  if (poll && String(poll.id) === pollId && !poll.resolved)
    return labels(poll.options);

  return [];
}

const tallyOf = (store, pollId) => {
  const ballots = ((store && store.ballots) || {})[pollId] || {};
  const counts = {};
...
const allTallies = (store) => {
  const out = {};
  for (const pollId of Object.keys((store && store.ballots) || {}))
    out[pollId] = tallyOf(store, pollId);
  return out;
};

/** Anonym vælger-id i en signeret cookie. Den kan ikke forfalskes, og den
 *  indeholder intet om personen — kun et tilfældigt tal. Rydder man cookies,
 *  kan man stemme igen; det er prisen for ikke at kræve login. */
function voterFrom(req, auth) {
  if (!auth || !auth.secret) return { id: null, cookie: null };
  const raw = /rdr2_voter=([^;]+)/.exec(req.headers.cookie || "")?.[1];
  if (raw) {
    const [id, signature] = decodeURIComponent(raw).split(".");
    if (id && signature) {
      const expected = sign(id, auth.secret);
      if (
        signature.length === expected.length &&
        crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expected))
      )
  ...
  const value = `${id}.${sign(id, auth.secret)}`;
  return {
    id,
    cookie: `rdr2_voter=${encodeURIComponent(value)}; HttpOnly; Path=/; SameSite=Lax; Max-Age=2592000${isSecure(req) ? "; Secure" : ""}`,
  };
}

/* Secure på sessionscookien, så snart forbindelsen er HTTPS. Den sættes IKKE
   ubetinget: så ville browseren smide cookien væk på http://localhost, og man
   kunne ikke længere logge ind under udvikling. I produktion står serveren bag
   en proxy, der sætter x-forwarded-proto. */
const isSecure = (req) =>
...
async function handleApi(req, res, url) {
  ...
  if (route === "/api/vote" && method === "POST") {
    ...
    // Kun state bestemmer, hvad man kan stemme på. Før validerede vi intet,
    // så man kunne opfinde afstemninger og — værre — sende "previous" og
    // dermed trække andres stemmer ned. Feltet bruges ikke længere.
    const state = await readJSON(statePath(), {});
    const legal = legalOptions(state, poll);
    if (!legal.length)
      return send(res, 404, { error: "afstemningen er ikke åben" });
    if (!legal.includes(option))
      return send(res, 400, { error: "ukendt svarmulighed" });

    const { id: voter, cookie: voterCookie } = voterFrom(
      req,
      await readJSON(authPath(), null),
    );
    if (!voter) return send(res, 503, { error: "kan ikke stemme lige nu" });

    // Én stemmeseddel pr. browser pr. afstemning. Stemmer man om, flyttes
    // sedlen — der lægges ikke en ny oveni.
    const store = await updateJSON(votesPath(), {}, (current) => {
      const ballots = current.ballots || {};
      ballots[poll] = { ...(ballots[poll] || {}), [voter]: option };
  ...
  if (route === "/api/admin/login" && method === "POST") {
    ...
    return send(
      ...
      { ok: true },
      {
        "Set-Cookie": `rdr2_session=${session}; HttpOnly; Path=/; SameSite=Strict; Max-Age=43200${isSecure(req) ? "; Secure" : ""}`,
      },
    );
  }
  ...
  // Admin ser optællingen, ikke de enkelte stemmesedler. Sedlerne siger kun
  // hvilken anonym browser der stemte hvad, og det rager ikke nogen.
  if (route === "/api/admin/votes" && method === "GET")
    return send(res, 200, allTallies(await readJSON(votesPath(), {})));

  if (route === "/api/admin/votes/reset" && method === "POST") {
    const body = (await readJsonBody(req)) || {};
    const poll = clean(body.poll, 60);
    await updateJSON(votesPath(), {}, (current) => {
      const ballots = { ...(current.ballots || {}) };
      if (poll) delete ballots[poll];
      return { ...current, ballots };
    });
    return send(res, 200, { ok: true });
  }
...
function isPrivate(full) {
  ...
  // "data" spærres på navn og ikke kun på DATA-stien: flyttes DATA_DIR ud af
  // webroden, må en glemt ./data-mappe stadig ikke kunne hentes.
  const parts = path.relative(ROOT, full).split(path.sep);
  return parts.includes("source") || parts.includes("data");
}
...
async function serveStatic(req, res, url) {
  ...
  try {
    ...
    // En mappe serveres som sin index.html — det er sådan /marcel og
    // /powerwalk virker uden at skulle skrives med /index.html til sidst.
    // Cloudflare Pages gør det samme, så de to opfører sig ens.
    if (stat.isDirectory()) {
      ...
      const inner = await fsp.stat(target);
      if (!inner.isFile()) throw new Error("ingen index");
    }
    const ext = path.extname(target);
    // Overlays skal kunne indlejres i OBS; resten må ikke kunne rammes ind.
    const isOverlay = path.basename(target) === "overlay.html";
    // Marcel-siden henter klip-thumbnails fra Twitchs billed-CDN og forbinder
    // til Twitchs IRC efter samtykke. Samme undtagelse som /marcel/* i
    // _headers — ellers virker siden i produktion, men ikke lokalt.
    const isMarcel = file === "/marcel" || file.startsWith("/marcel/");
    const twitchImg =
      " https://static-cdn.jtvnw.net https://clips-media-assets2.twitch.tv";
    res.writeHead(200, {
      ...
      "Content-Security-Policy": [
        ...
        `img-src 'self' data:${isMarcel ? twitchImg : ""}`,
        "font-src 'self'",
        `connect-src 'self'${isMarcel ? " wss://irc-ws.chat.twitch.tv" : ""}`,
